---
title: "ompBAM-API-Docs"
output:     
  rmarkdown::html_document:
    toc: true
vignette: >
  %\VignetteIndexEntry{ompBAM-API-Docs}
...
# Setting up a new R package to include ompBAM
...
utilise ompBAM. ompBAM uses zlib to decompress BAM files. So you must link
your source code to Rcpp, zlibbioc as well as ompBAM itself.
...
# Writing a simple program based on ompBAM

In this vignette, we will demonstrate how to use ompBAM to open a BAM file and
read its contents in a multi-threaded way. In essence, we will explain 
step-by-step how the code works in the ompBAMExample package included with this
package.

To install the ompBAMExample package, refer to the following:
...
To add the ompBAM header files into your source code (here we refer to 
"main.cpp") in ompBAMExample package, start with the following:
...
The line #include "Rcpp.h" is probably redundant as ompBAM.hpp already contains
this line, but we include it here for completeness.

## Adding progress bar functionality

ompBAMExample also demonstrates how to use a progress bar to visualise the
...
```
LinkingTo: Rcpp, zlibbioc, ompBAM, RcppProgress
```
...
ompBAM, we read the BAM file sequentially using multiple threads. Thus, we do
...
## Opening a BAM file using ompBAM

ompBAM uses the standard library <fstream> to read BAM files. <fstream> is
already included within the ompBAM headers. To open a BAM file, we must open
...
ompBAM uses two functions, fillReads() and supplyRead(), to read BAM files and
...
the RcppProgress functions with ompBAM to provide a progress bar.
