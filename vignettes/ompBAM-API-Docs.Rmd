...
zlibbioc is now removed as a dependency. Users wishing to
update their packages without zlibbioc should:
- remove zlibbioc as a dependency on DESCRIPTION file
- remove any reference to zlibbioc in their R files
- edit Makevars.win (see section 1c)
...
with OpenMP. You can view these files from within the
`src/` directory.
...
Note that `-lz` will link to the zlib libraries. This will 
ensure that the zlib libraries are linked correctly to Windows
systems.
