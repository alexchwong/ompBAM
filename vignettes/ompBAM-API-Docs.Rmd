...
abstract:
    ...
    R/Bioconductor environment. Parallel processing in R is achieved using 
    BiocParallel or related packages. Typical alternatives include processing
    multiple BAM files, each using a single core. Although easy to set up, 
    memory-intensive applications limit such approaches. *ompBAM* is a 
    header-only C++ library based on OpenMP, allowing developers to implement
    OpenMP-based parallel processing for the sequential reading of BAM files. 
    *ompBAM* makes it easy by handling all the parallel file-access and bgzf
    decompression, allowing developers to focus on multi-threaded handling of 
    individual alignments / reads.
...
To install ompBAM, start R (version "4.1") and enter: 
...
desired project to the variable `project_path`), use RStudio to open the 
...
Also, the NAMESPACE file needs to be changed to import Rcpp and zlibbioc. This
is required to link the compiled dll of the package. `use_ompBAM()` will have 
already created roxygen2 tags inside the `ompBAM_imports.R` file within the `R/`
directory.
...
detail and explain how to use ompBAM to re-create the *idxstats* function.
...
Note that we use `:::` to call functions exported from the C++ source code by
Rcpp. This is because, although Rcpp has exported the `idxstats_pbam` function,
this is currently only accessible to the R source code in your package. You will
still need to create wrapper functions from within R, as covered later in 
section (2c).
...
The line `#include "Rcpp.h"` is probably redundant as `<ompBAM.hpp>` already 
includes `Rcpp.h`, but we include it here for completeness.

The `cout` C++ function is not supported in R, and is replaced by `Rcout` in 
Rcpp. For compatibility with non-R environments, all output messages in ompBAM
uses `cout`. The line `#define cout Rcpp::Rcout` ensures that `cout` is
interpreted by the compiler as `Rcpp::Rcout`.
...
their contents can not be accessed by other threads processing other iterations.
...
read_storage.push_back(real_read.realize);
```
...
read the BAM file. This is used as an alternative to `pbam_in::openFile()`.
...
Reads are divided into contiguous blocks. Thus, thread `i=0` contains a
...
* `GetProgress()` Returns the total number of bytes in the BAM file that has
been processed (i.e. read and decompressed)
...
the reads to make the `pbam2_t` data persist. As explained previously in
